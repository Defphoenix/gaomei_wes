...
两套突变结果需要按 HGVS 和捕获 BED 进行差异审计时，可使用
[`scripts/compare_variant_reports.py`](scripts/compare_variant_reports.py)。脚本输出逐条对齐
TSV、Excel 工作簿和中文 HTML 汇报，并明确区分“BED 外”与“因缺少基因组坐标而无法判定”。
...
| 7e | `07e_manual_filter.sh` | Python, BCFtools | 变异 | VEP后按配对证据和群体AF执行可审计人工硬过滤 |
...
│   ├── 07c_vep.sh               # VEP注释
│   ├── 07e_manual_filter.sh     # VEP后人工硬过滤
│   ├── manual_filter_vep.py     # 阈值过滤与逐位点审计
...
| 人工过滤 | `annotation/*.vep.manual_filtered.vcf.gz` | 通过人工阈值的VEP注释VCF；新抗原和TMB默认读取 |
| 人工过滤 | `annotation/*.vep.manual_filter_audit.tsv`, `*_summary.json` | 每项阈值判定、失败原因、参数和计数 |
...
RUN_VEP=true        # VEP注释
RUN_MANUAL_FILTER=true # VEP后人工硬过滤
...
```bash
# 在capture BED每个区间两侧增加100 bp作为Mutect2局部组装上下文。
# 这会扩大调用/过滤搜索区间，但不会改写原BED，也不会改变TMB分母。
MUTECT2_INTERVAL_PADDING=100

# VEP完成后执行；群体AF缺失时按通过处理。
...
`7e` 不按基因或 consequence 排除变异，只判断技术证据和群体频率。TMB步骤随后再按
VEP consequence、有效编码区和分母规则筛选；新抗原则默认从人工过滤后的VEP VCF生成。

### 新抗原模块

```bash
# VEP和7e完成后运行。留空时优先读取 *.vep.manual_filtered.vcf.gz
NEOANTIGEN_VEP_VCF=""
