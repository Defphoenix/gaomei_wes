...
MUTECT2_INTERVAL_PADDING=100              # 调用BED两侧增加局部组装上下文；不改变TMB分母BED
...
#---------------------------------------
# VEP后人工体细胞硬过滤
# 群体AF依赖VEP CSQ，因此本步骤必须位于VEP注释之后。
#---------------------------------------
RUN_MANUAL_FILTER=true
MANUAL_FILTER_VEP_VCF=""                 # 留空时读取当前样本VEP VCF
...
MANUAL_FILTER_MAX_POPULATION_AF=0.001     # 0.10%；缺失群体AF按通过处理
...
SKIP_MANUAL_FILTER=false                  # 仅Mutect2且VEP启用时执行
